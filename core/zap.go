...
func InitZap() (logger *zap.Logger) {
	...
	cores := [...]zapcore.Core{
		getEncoderCore("server_debug", debugPriority),
		getEncoderCore("server_info", infoPriority),
		getEncoderCore("server_warn", warnPriority),
		getEncoderCore("server_error", errorPriority),
	}
	logger = zap.New(zapcore.NewTee(cores[:]...), zap.AddCaller())

	logger = logger.WithOptions(zap.WithCaller(global.CONFIG.Zap.ShowLine))
...
func getEncoderCore(fileName string, level zapcore.LevelEnabler) (core zapcore.Core) {
	writer, _ := utils.FileRotatelogs.GetWriteSyncer(fileName) // 使用file-rotatelogs进行日志分割
